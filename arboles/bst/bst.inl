...
void bst<K,V>::removePri(node<K,V>** n,V x){
    ...
    if(x <(*n)->dato) 
    { 
         removePri(&(*n)->p_child[0],x);//Si X es inferior al valor del nodo, sigue en el árbol de la izquierda elimina el nodo X 
         if(2==altura((*n)->p_child[1])-altura((*n)->p_child[0])) 
            if((*n)->p_child[1]->p_child[0]!=NULL&&(altura((*n)->p_child[1]->p_child[0])>altura((*n)->p_child[1]->p_child[1])) ){
                doblerotaciondere(n); 
    ...
    { 
         removePri(&(*n)->p_child[1],x);//Si X es mayor que el valor del nodo, sigue en el nodo eliminar X subárbol 
         if(2==altura((*n)->p_child[0])-altura((*n)->p_child[1])) 
            if((*n)->p_child[0]->p_child[1]!=NULL&& (altura((*n)->p_child[0]->p_child[1])>altura((*n)->p_child[0]->p_child[0]) )) {
    ...
    { 
        ...
        { 
            node<K,V>* temp=(*n)->p_child[1];//Nodos hijo temp hacia la derecha 
            while(temp->p_child[0]!=NULL) temp=temp->p_child[0];//Encontrar un mínimo de nodos subárbol mediana 
            //A la derecha en el valor mínimo de nodos subárbol de asignar a este nodo 
            (*n)->dato=temp->dato; 
            (*n)->freq=temp->freq; 
            removePri(&(*n)->p_child[1],temp->dato);//Eliminar nodos en el valor mínimo de la derecha subárbol 
            if(2==altura((*n)->p_child[0])-altura((*n)->p_child[1])) 
            { 
                if((*n)->p_child[0]->p_child[1]!=NULL&& (altura((*n)->p_child[0]->p_child[1])>altura((*n)->p_child[0]->p_child[0]) )){
                    doblerotacionizqui(n);
